# This code is used to generate bandscale embedding.

library(BandNorm)
input_path = ""
save_path = ""

bandscale = function(path = NULL, hic_df = NULL, save = TRUE, save_path = NULL) {
  ...
  hic_df <- hic_df %>% left_join(band_info, by = c("chrom", "diag", "cell")) %>% mutate(BandNorm = count/band_depth) %>% 
...
output = bandscale(input_path, save = TRUE, save_path = save_path)
